---
title: "[Data Structure] 스택과 큐, 덱 Stack & Queue"
...
## 스택이란 ? 
스택은 삽입과 삭제 연산이 <span style="background-color: #fff3cd"> 후입선출 _Lifo_ (Last-in-First-out) </span> 로 이루어지는 자료구조다! <br>
1->2->3의 순서로 입력되었다면, 3->2->1의 순서로 출력되는 것임
![문제 풀이](/assets/images/stack.png)

- 특징 :
  - 삽입과 삭제가 <span style="background-color: #fff3cd">한 쪽에서만</span> 일어난다 -> top을 통해서만 접근 가능
...
- 특징 :
  - 삽입과 삭제가 <span style="background-color: #fff3cd">양방향에서</span> 일어난다 -> 한 쪽에서 삽입 작업이 이루어지고, 다른 한 쪽에서는 삭제 작업이 이루어짐
  - <span style="color: blue"> 너비 우선 탐색 BFS</span> 에서 자주 사용하는 개념
  - 일반적으로 파이썬에서 **deque를 이용**해서 큐를 구현

- 연산 :
  - _rear : 큐에서 가장 끝, 삽입(enQueue)만 이루어짐_
  - _front : 큐에서 가장 앞, 삭제(dnQueue)만 이루어짐_
  - <span style="background-color: #fff3cd">lst.append(data) : rear 부분에 새로운 데이터를 삽입하는 연산</span>
  - lst.appendleft(): 왼쪽에서 데이터를 삽입
  - <span style="background-color: #fff3cd">lst.popleft() : front 부분에 있는 데이터를 삭제하고 확인하는 연산</span>
  - lst.pop(): 오른쪽에서 데이터 삭제
...
deque([3])
```
FIFO구조이므로 가장 먼저 append된 1부터 pop되어 [1]->[1,2]->[1,2,3]->[2,3]->[3] 이런식이 되는 구조다. 
...
## 덱이란?
<span style="background-color: #fff3cd">양쪽에서 삽입과 삭제가 가능한 구조이며 스택과 큐의 연산을 모두 지원하는게 덱(Dequeue)!</span>
<br> <br>
...
요런 자료구조들에서 주의할 것들 ! <br>
오버플로: 특정한 자료구조가 수용할 수 있는 데이터의 크기를 넘어가는 삽입 연산을 수행할 때 발생한다.<br><br>

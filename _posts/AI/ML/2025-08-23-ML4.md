...
- 보통 기준으로 삼는 수 = 0.5
- 따라서, $ h_\theta(x) = 0.5 , \theta^Tx = 0 $


### 🐾 선형 결정 경계
예를 들어, 입력 특징이 2개 ($x_1, x_2$)라면:<br>
<span style="font-size:200%"> $ h_\theta(x) = g(\theta_0 + \theta_1 x_1 + \theta_2 x_2) $</span><br>
- 이때, 결정 경계는 $ \theta_0 + \theta_1 x_1 + \theta_2 x_2 = 0 $
  
### 🐾 비선형 결정 경계
현실 데이터는 직선으로 나누기 어려운 경우가 많으니, 입력 특징을 feature mapping 해서 비선형 항을 추가하자!<br>
<br>
예를 들어 :
<span style="font-size:200%"> $ x \mapsto \phi(x) = [1,\, x_1,\, x_2,\, x_1x_2,\, x_1^2,\, x_2^2, \dots] $</span><br>
이러면 $ h_\theta(x) = g(\theta^T \phi(x)) $
- $x_1^2$, $x_2^2$ 같은 항을 추가하면  
  **원형, 곡선, 타원** 형태의 경계도 학습할 수 있음.

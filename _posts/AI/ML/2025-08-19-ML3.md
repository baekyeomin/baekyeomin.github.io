...
> 각각을 최대값으로 나누어서 [0,1] 범위로 정규화하는 방법
> 수식 : <span style=color: blue>$x_j' = \frac{x_j - \min(x_j)}{\max(x_j) - \min(x_j)}$</span>
> - $\min(x_j)$ : j번째 특징의 최소값  
> - $\max(x_j)$ : j번째 특징의 최대값  
> <br>
> <br>
> **2. Mean Normalization (평균정규화)**
> 특징(feature)의 평균을 0에 가깝게 하는 방법으로, 범위를 [-1,1] 정도로 맞추는 방법
> 수식 : <span style=color: blue>$x_j' = \frac{x_j - \mu_j}{\max(x_j) - \min(x_j)}$</span>
> - $\mu_j$: j번째 특징의 평균값  
> - <span style=color: red>$x_0 = 1$ (bias 항)은 정규화 적용하지 않음! bias term은 고정된 상수임! </span>
...
>
> **2. 학습률이 너무 작은 경우** <br>
> - $J(\theta)$가 안정적으로 감소하긴 하지만<span style=color: blue> **매우 느리게 수렴** </span>
> - 계산 속도 느림

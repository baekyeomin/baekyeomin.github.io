...
두 벡터의 내적 $q_i^{T} p_u$ 은 사용자 u와 아이템 i 사이의 interaction, 즉 아이템 특성 전반에 대한 사용자의 전체적인 선호도를 나타냅니다. 이 값은 사용자 u가 아이템 i에 줄 평점 $\hat{r}_{ui}$ 를 근사한다고 하며, 식으로 표현하면 $\hat{r}_{ui} = q_i^{T} p_u$ 입니다. 

---
title: "[Paper Review] #02 Matrix Factorization Techniques for Recommender Systems"
...
그럼 모든 신호를 통합한 확장 모델은 다음과 같습니다. <br>

![수식6](/assets/images/수식6.png) 
...
## 8. Temporal Dynamics
OTT에 새로운 콘텐츠가 등록될 수 있고, 시간이 지나면 item의 인기도가 변할 수 있으며, 사용자 취향 또한 시간에 따라 변합니다.<br>
 따라서 <span style="background-color: #fff3cd">추천 시스템은 사용자–아이템 상호작용의 시간에 따른 변화(temporal effects)를 고려</span>해야합니다. <br>
<br>
Matrix Factorization 모델은 이러한 시간적 효고를 모델링 하기에 적합하다고 합니다. <br>
평점을 여러 구성요소로 분해하면 서로 다른 시간적 요인을 각각 따로 처리할 수 있다고 합니다. <br>

### 🐾 아이템 인기도의 변화
item의 인기도는 시간에 따라 변합니다. <br>
따라서 모델은 아이템 편향 $b_i(t)$을 시간의 함수로 취급합니다. 

### 🐾 사용자 평점 기준의 변화
예를 들어 과거에는 평균적으로 4점을 주던 사용자가 시간이 지나면서 3점을 주기 시작할 수도 있습니다. <br>
(평점 주는 기준이 자연스럽게 변했다던지..최근에 본 콘텐츠들과 비교하여 평가하는 경향이 있다던지...가족 구성원 변화 등으로 계정 사용자가 바뀌던지..  등등) <br>
따라서 사용자 편향 $b_u$ 또한 시간의 함수로 모델링합니다. 

### 🐾 사용자 취향 변화
단순히 평점 기준뿐 아니라 **취향 자체**에도 영향을 줄 수 있습니다. <br>
예를 들어 스릴러 영화를 좋아하던 사용자가 1년 후 범죄 드라마를 더 선호하게 될 수도 있고 특정 배우나 감독의 팬이 될 수도 있습니다. <br>
<span style="color: red"> 이를 반영하기 위해 사용자 요인 벡터 $p_u$ 역시 시간의 함수로 취급합니다.<br> 
면 아이템 특성 $q_i$는 인간과 달리 시간이 지나도 변하지 않는 정적인 특성으로 봅니다.</span> 

### 평점 예측 식 :
시간에 따라 변하는 파라미터를 사용하면 평점 예측식은 다음과 같습니다. <br>

![수식7](/assets/images/수식7.png) 


## 9. Inputs with Varying Confidence Levels
여러 상황에서 관측된 모든 평점이 동일한 신뢰도를 가지는 것은 아닙니다. <br>
예를 들어 특정 아이템의 평점을 의도적으로 높이거나 낮추려는 악의적인 사용자(adversarial users)가 존재한다면 신뢰도는 낮을 것입니다. <br> 또, 광고가 이루어진 item에 대해서 rating이 높더라도 이 rating의 신뢰도는 낮다고 볼 수 있습니다. 

> 그러나 이 부분에서 제가 잘 못 이해한 점이 있었습니다...<br>
> confidence level이라는것이 광고 여부.. 등에 따라 신뢰도가 떨어지네 마네가 아니라 <span style="color: red"> Implicit feedback 환경에서는 관측값이 0/1로 주어지는데 여기서 1은 사용자가 해당 아이템을 “좋아한다”는 의미가 아니라 단순히 상호작용이 발생했다(observed) 는 의미에 가깝기 때문에 </span> 사용자가 아이템을 클릭했다고 해서 반드시 선호한다고 볼 수 없고,
반대로 클릭하지 않았다고 해서 싫어한다고 단정할 수도 없다는 점에서 MF에서 confidence level을 가중치로 사용하여 더 신뢰할 수 있는 관측에 더 큰 영향을 주도록 학습하는 것이었습니다. ...
<br>

그리하여 confidence level을 반영한 수식은 아래와 같습니다. 

![수식8](/assets/images/수식8.png) 


## 10. Netflix Prize Competition
Netflix는 추천시스템 성능 개선을 위해 2006년에 대회를 개최했었습니다. <br>
공개한 데이터는 다음과 같았습니다. 
- 약 100M개의 평점
- 약 500,000명의 사용자
- 17,000개 이상의 영화
...
참가팀은 약 3M개의 테스트 평점에 대한 예측값을 제출하는 방식이었고, 평가 지표는 **RMSE (Root Mean Square Error)** 이었으며, 총 48000개 이상의 팀이 182개국에서 참여했다고 합니다. 

### 🐾 이 중 BellKor 팀의 성과
- 2007: 8.43% 개선 → Progress Prize 수상
- 2008: Big Chaos 팀과 협력 → 9.46% 개선
- 100개 이상의 예측 모델 앙상블 사용
- 대부분이 **Matrix Factorization 기반 모델**


### 🐾 MF의 핵심 아이디어
<span style="background-color: #fff3cd">user-item rating matrix를 분해하여 latent factor을 발견하는 것 </span>

<br> <br>

![netflixprizecompetition](/assets/images/netflixprizecompetition.png) 


### 🐾 모델 개선 과정

1. 그냥 MF
2. Bias 추가한 버전 (User / item)
3. implicit feedback 추가한 버전
4. temporal dynamics 추가한 버전 1
5. temporal dynamics 추가한 버전 2 <br>
로 실험을 진행하였습니다. 


![netflixprizecompetition2](/assets/images/netflixprizecompetition2.png) 

그 결과, 파라미터 수가 증가할 수록 차원이 증가하여 RMSE가 감소 하였습니다. <br>
또, 더 복잡한 모델일수록 정확도가 향상되었고, 특히 시간적 변화 모델링이 매우 중요함을 알게 되었습니다. <br> <br> 
Netflix 시스템 RMSE: 0.9514  <br> 
Grand Prize 목표: 0.8563 


## 쫑알 쫑알
요약하면 MF는.. <br> 
- 최근접 이웃(Nearest Neighbor) 방식보다 높은 정확도
- 메모리 효율적
- 학습이 비교적 용이  
  - Bias
  - Implicit feedback
  ...
  - 다양한 피드백 형태
<br> <br> <br> 
교수님께서 발표를 깔끔하게 잘했다고 칭찬 받아서 무지무지 기분이 좋았던 랩미팅이었습니다!! 😁😁 <br> 

> 다만 교수님께서 **병렬화 용이성 여부 관점에서 SGD와 ALS 간의 차이**에 대해 정리하여 메일을 보내달라고 하셨고, 이에 대해 아래와 같이 작성하여 보내드렸습니다. <br> <br>
>
> SGD는 하나의 평점 데이터 (u,i)에 대해 user 벡터와 item 벡터를 동시에 업데이트를 하는 구조입니다. <br>
이때 여러 연산이 동시에 수행되면, 동일한 파라미터를 함께 업데이트하게 되어 같은 메모리를 동시에 수정하게 되면서 충돌이 일어날 수 있습니다. <br>
예를 들어 한 사용자가 여러 영화에 대해 평점을 남긴 경우, 연산1이 (user1, item3)을 업데이트하고, 연산 2가 (user1, item7)을 업데이트할 때, <br>
두 연산이 동시에 같은 user1 벡터를 읽고 업데이트하면 나중에 저장한 값이 먼저 저장한 값을 덮어써서 일부 정보를 잃을 수 있습니다. <br>이러한 상황을 race condition이라고 합니다.<br>
이 문제를 방지하려면 각 파라미터 업데이트 과정에서 lock(한 번에 하나에만 접근하도록 잠그는 것)이나 동기화(여러 연산의 실행 순서를 맞춰 충돌을 방지하는 것)가 필요한데, 이러한 방식은 병렬 처리 과정에서 속도가 느리게하여 병렬화의 이점이 많이 사라진다고 합니다.<br>
(이 내용은 Hogwild!: A Lock-Free Approach to Parallelizing Stochastic Gradient Descent(2011) 논문의 초록 부분을 참고하였습니다.) 
<br><br>
반면에 ALS는 user 벡터와 item 벡터를 번갈아 고정한 뒤 각각을 최소 제곱 문제로 독립적으로 계산하기 때문에 user벡터를 계산할 때 다른 user 벡터가 필요하지 않고 서로 다른 사용자들끼리의 계산이 서로 영향을 주지 않습니다. <br>
따라서 user벡터나 item 벡터를 여러 묶음으로 나누어 각각 다른 CPU나 머신에서 동시에 계산할 수 있습니다.<br>
이러한 차이로 인해 대규모 분산 환경에서는 ALS가 SGD에 비해 병렬화에 더 용이하다는 장점이 있다고 생각합니다. <br>

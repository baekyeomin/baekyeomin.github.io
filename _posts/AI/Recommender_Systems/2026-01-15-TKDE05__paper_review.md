...
> <br>
> 분모로 가중치의 합을 나누어 주는 것은 벡터의 크기를 정규화하여, item 개수에 상관없이 사용자 간 비교가 가능하도록 하기 위함입니다.
...
> A. 데이터과학에서 배운 바로는 베이지안 분류기는 P(클래스 | 데이터) 형태로, 어떤 데이터가 주어졌을 때, 그 데이터가 어떤 클래스일 확률이 얼마일지 구하는 모델입니다. <br>
> 예를 들어 데이터과학에서 배웠던 스팸 메일 분류라면, 과거 메일들을 보고 각 키워드 마다 스팸 메일에서 얼마나 자주 나왔는지 확률을 계산해서 학습하고 새 메일이 왔을 때 어떤 키워드로 구성되어 있는지 보고 스팸일 확률을 계산합니다. 이후 확률이 가장 큰 클래스로 분류합니다. <br> 이를 추천시스템에 그대로 적용한다면, 어떤 item을 이루고 있는 키워드들 (어쩌구저쩌구) 그런데 이때 나이브 베이즈 같은 경우 키워드(feature)들끼리 서로 독립이라고 가정해서 계산을 더 단순화한것입니다. 
>
> <br>
>
...
> <span style="color: red">여기서 교수님께서 질문한 부분: </span> <br> 
> **Q. CF에서는 item에 대한 정보 중에 무엇이 활용될 수 있나요?**
...
> <br>
> **Q. 그렇다면 title이 같은 item 같은 경우에는 어떻게 구분 가능할까요?**
...
- (b) 유사도 가중 평균 방식  
  : 사용자 간 유사도를 가중치로 사용하여  
  이웃 사용자들의 평점을 가중 평균  

- (c) 평점 기준 보정 방식  
  : 사용자마다 다른 평점 기준(rating scale)을 고려하여  
  각 사용자의 평균 평점 대비 **편차(deviation)** 를 사용 